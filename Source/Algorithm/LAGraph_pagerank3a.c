...
// format.  All entries in A must be equal to 1.  All nodes of A must have at
// least one out-going edge (that is, the matrix cannot have any empty rows).
// For fastest results, the matrix A should not have any empty columns.
...
)
{
    ...
    for ((*iters) = 0 ; (*iters) < itermax && rdiff > tol ; (*iters)++)
    {
        ...
        // rdiff = sum (|pr-oldpr|)
        LAGRAPH_OK (GrB_eWiseAdd (oldpr, NULL, NULL, op_diff, oldpr, pr,
            NULL)) ;
        LAGRAPH_OK (GrB_reduce (&rdiff, NULL, GxB_PLUS_FP32_MONOID, oldpr,
            NULL)) ;
        GrB_free (&oldpr) ;
   }
