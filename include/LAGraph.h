...
#define LAGRAPH_DATE "Aug 31, 2022"
 ...
 *
 * (1) coordinate:  for this format, the first non-comment line must appear,
 ...
 * types.
 *
 * (2) array: for this format, the first non-comment line must appear, and it
 ...
 * terms INF, +INF, -INF, and NAN to represent floating-point infinity and NaN
 * values, in either upper or lower case.
 *
 * The <storage> token is general, symmetric, skew-symmetric, or Hermitian:
 *
 * (1) general: the matrix has no symmetry properties (or at least none that
 * were exploited when the file was created).
 *
 * (2) symmetric:  A(i,j) == A(j,i).  Only entries on or below the diagonal
 * appear in the file.  Each off-diagonal entry in the file creates two entries
 * in the GrB_Matrix that is returned.
 *
 * (3) skew-symmetric:  A(i,j) == -A(i,j).  There are no entries on the
 ...
 * returned.
 *
 * (4) Hermitian: square complex matrix with A(i,j) = conj (A(j,i)).  All
