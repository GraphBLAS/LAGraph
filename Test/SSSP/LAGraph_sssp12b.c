...
)
{
    ...
    LAGr_Vector_new(&sum_s, GrB_BOOL, n);

    // t = infinity, t[src] = 0
    LAGr_assign(t, NULL, NULL, INT32_MAX, GrB_ALL, n, NULL);
    LAGr_Vector_setElement(t, 0, source);

    // sum_s = true, t[src] = false,
    // where true indicates the node could be further relaxed
    LAGr_assign(sum_s, NULL, NULL, true, GrB_ALL, n, NULL);
    LAGr_Vector_setElement(sum_s, false, source);
    GrB_Index ToBeRelax = n;
    ...
    while (tmasked_nvals > 0 || moreBucket)
    {
        //printf ("\n============================= outer: %d\n", i) ;
        ...
        if (moreBucket)
        {
            LAGr_select(tmasked, NULL, NULL, GxB_LT_THUNK, tmasked, uBound,
                NULL);
        }
        else
        {
            LAGr_Vector_clear (tmasked);
            //LAassign (tmasked, sum_s, NULL, t, GrB_ALL, n, NULL);
            LAGr_select(tmasked, sum_s, NULL, GxB_LT_THUNK, t, uBound, NULL);
        }
        t1 = LAGraph_toc(tic);
        total_time1 += t1;
        ...
        // if the current bucket is empty and the previous found left bucket
        // could have been finished, then we need to check if any bucket left
        if (tmasked_nvals == 0 && !moreBucket)
        {
            LAGr_select(tmasked, sum_s, NULL, GxB_LT_THUNK, t, Inf, NULL);
            LAGr_Vector_nvals(&tmasked_nvals, tmasked);
            if (tmasked_nvals != 0)
            {
                moreBucket == true;
        ...
        else if (tmasked_nvals != 0)
        {
            while (tmasked_nvals > 0)
            {
                ...
                if (print_lvl > 0)
                {
                    printf ("inner tmasked has %ld nnz\n",tmasked_nvals);
                }
            }
            ...
            LAGraph_tic (tic);
            LAGr_assign(sum_s, s, NULL, false, GrB_ALL, n, GrB_DESC_S);
            LAGr_Vector_nvals(&s_nvals, s);
            ToBeRelax -= s_nvals;
            tmasked_nvals = ToBeRelax;
            t1 = LAGraph_toc(tic);
            total_time7 += t1;
            // GxB_print(tmasked, print_lvl);
