//------------------------------------------------------------------------------
// LAGraph_sssp12a: single-source shortest path, copy from sssp12 with effort
// to reduce the times of performing tmasked = select(t<Inf)
//------------------------------------------------------------------------------
...
)
{
    ...
    // if there is more bucket(s) left to compute
    bool moreBucket = true;
    ...
    double t1 ;
    int selectLTInf = 0, total_iter = 0;
    ...
    while (tmasked_nvals > 0)
    {
        ...
        if (tmasked_nvals != 0)         // if the current bucket is not empty
        {
            //------------------------------------------------------------------
            ...
            while (tmasked_nvals > 0)
            {
                ...
                // later is longer than using eWiseAdd here Time taken here and
                // time to get tmasked =(s.*t) is commented below for kron
                // matrix s = (s | pattern of tmasked)
                LAGraph_tic (tic);
            ...
            // Assume the current bucket is the last one
            moreBucket = false;

            //------------------------------------------------------------------
            // prepare for the next loop, and find out how many left to be
            // computed
            //------------------------------------------------------------------
        ...
        else          // if the current bucket is empty
        {
            //------------------------------------------------------------------
            // prepare for the next loop, and find out how many left to be
            // computed
            //------------------------------------------------------------------
            printf(" current bucket is empty\n");
            ++i;

            LAGraph_tic (tic);
            LAGr_Scalar_setElement (lBound, i * delta) ;
            // if we know there is at least one bucket left, and we keep finding
            // empty buckets, then there is no need to keep performing
            // tmaksed = select (t < infinity). Instead, simply perform
            // tmasked = select (t >= i*delta)
            if (moreBucket)
            {
                LAGr_select (tmasked, NULL, NULL, GxB_GE_THUNK, t, lBound,
                    NULL) ;
                LAGr_Vector_nvals (&tmasked_nvals, tmasked) ;
            }
            // if the previously found bucket has been possibly processed,
            // and the current bucket is empty, we need to make sure if there
            // is any more bucket left by finding
            // tmasked = select (i*delta <= t < infinity)
            else
            {
                printf(" selecting LT Inf\n");
                selectLTInf++;
                GrB_Index tmasked_nvals_tmp ;
                // try to make tmasked as sparse as possible in the first select
                if (do_LT_first)
                {
                    LAGr_select (tmasked, NULL, NULL, GxB_LT_THUNK, t, Inf ,
                        NULL) ;
                ...
                if (tmasked_nvals > 0)
                {
                    moreBucket = true; // there is at least one bucket left
                }
            }
            t1 = LAGraph_toc(tic);
            total_time7 += t1;
    ...
    *path_length = t;
    ...
    printf("iter times %d, select LT Inf times: %d\n", total_iter, selectLTInf);
    return GrB_SUCCESS;
}
