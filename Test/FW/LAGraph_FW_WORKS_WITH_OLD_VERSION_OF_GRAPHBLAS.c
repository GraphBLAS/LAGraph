...
)
{
    ...
    GrB_Index n, ncols;
    OK (GrB_Matrix_nrows(&n, G));
    OK (GrB_Matrix_ncols(&ncols, G));
    if (n != ncols)
    {
        return (GrB_INVALID_VALUE) ;
    }
    ...
    OK (GrB_Matrix_new (D,  otype, n, n)) ;
    OK (GrB_Matrix_new (&A, otype, n, 1));
    OK (GrB_Matrix_new (&B, otype, 1, n));
 
    // D = G, with possible typecasting
    OK (GrB_assign (*D, GrB_NULL, GrB_NULL, G, GrB_ALL, n, GrB_ALL, n, GrB_NULL)) ;
    ...
    for (GrB_Index i = 0; i < n; i++)
    {
        ...
        OK (GrB_extract(A, GrB_NULL, GrB_NULL, *D, GrB_ALL, n, &i, 1, GrB_NULL));
        // B = D (i,:), the ith row
        OK (GrB_extract(B, GrB_NULL, GrB_NULL, *D, &i, 1, GrB_ALL, n, GrB_NULL));
        // D = min (D,A*B) with "*" being the min-plus semiring
        OK (GrB_mxm(*D, GrB_NULL, op, semiring, A, B, GrB_NULL));
    }
