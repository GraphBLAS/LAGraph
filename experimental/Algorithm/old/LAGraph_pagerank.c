...
)
{
    ...
    bool jumbled ;
    GrB_Index dsize ;
    LAGRAPH_OK (GxB_Vector_export_CSC (&d, &type, &n, &dsize, &nvals,
        &jumbled, &I, (void **) (&X), NULL)) ;
    #else
    LAGRAPH_OK (GxB_Vector_export (&d, &type, &n, &nvals,
        &I, (void **) (&X), NULL)) ;
    #endif
    #else
    nvals = n ;
...
//      LAGRAPH_OK (GrB_Vector_clear (t)) ;
    ...
    #ifdef GxB_SUITESPARSE_GRAPHBLAS
    #if GxB_IMPLEMENTATION >= GxB_VERSION (4,0,0)
    LAGRAPH_OK (GxB_Vector_export_Full (&r, &type, &n, (void **) (&X), NULL)) ;
    #else
