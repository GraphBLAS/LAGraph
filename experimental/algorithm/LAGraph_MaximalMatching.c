...
)
{
    ...
    GrB_Vector scores = NULL ;              // score vector for edges
    ...
    GRB_TRY (GrB_Vector_new (&scores, GrB_FP32, num_edges)) ;
    ...
    while (ncandidates > 0) {
        ...
        if (matching_type == 0) {
            ...
            // since we are using PLUS_ONE, the vector doesn't matter as long as it's a full vector of UINT64s.
            // so, just use the Seed vector
            GRB_TRY (GrB_mxv (node_degree, NULL, PLUS_ONE_SEMIRING_UINT64, E, Seed, GrB_DESC_R)) ;

            // for each edge, sums incident edges for each node. Need to subtract 2 from result to exclude the edge itself
            GRB_TRY (GrB_mxv (degree, candidates, NULL, PLUS_SECOND_SEMIRING_UINT64, E_t, node_degree, GrB_DESC_RS)) ;
            GRB_TRY (GrB_apply (degree, candidates, NULL, SUB_TWO_UINT64, degree, GrB_DESC_S)) ;

            GRB_TRY (GrB_eWiseMult (scores, candidates, NULL, GrB_DIV_FP32, Seed, degree, GrB_DESC_S)) ;
        ...
        GRB_TRY (GrB_mxv (max_node_neighbor, NULL, NULL, GrB_MAX_SECOND_SEMIRING_FP32, E, scores, GrB_DESC_R)) ;
