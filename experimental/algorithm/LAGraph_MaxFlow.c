...
//id.residual = 0 ;
  ...
  while(n_active > 0){
    ...
    if(iter % 12 == 0){
      ...
      GRB_TRY(GrB_apply(res_mat, NULL, NULL, GrB_GetResidual, R, GrB_DESC_R)) ;
      GRB_TRY(GrB_select(res_mat, NULL, NULL, GrB_VALUEGT_FP64, res_mat, 0, GrB_DESC_R)) ;
      GRB_TRY(GrB_transpose(res_matT, NULL, NULL, res_mat, GrB_DESC_R));
