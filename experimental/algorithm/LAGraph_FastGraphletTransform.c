...
LG_JIT_STRING(
void LG_FGT_sub_one_mult (int64_t *z, const int64_t *x) {
(*z) = (*x) * ((*x)-1) ;
}, LG_FGT_SUB_ONE_MULT_DEFN)
...
//  GRB_TRY (GrB_Vector_new (&d_1, GrB_INT64, n)) ;
    ...
    GRB_TRY (GxB_UnaryOp_new (
        &Sub_one_mult, F_UNARY (LG_FGT_sub_one_mult), GrB_INT64, GrB_INT64,
        "LG_FGT_sub_one_mult", LG_FGT_SUB_ONE_MULT_DEFN)) ;
...
#define TRY(method)                         \
    ...
    for (i_tile = 0; i_tile < tile_cnt; ++i_tile)
    {
        ...
        TRY (GrB_mxm (A_i, NULL, NULL, LAGraph_plus_one_int64, A_Tiles [i_tile], A, NULL)) ;
        ...
        TRY (GrB_mxm (C_Tiles [i_tile], NULL, NULL, LAGraph_plus_first_int64, A_i, e, NULL)) ;
