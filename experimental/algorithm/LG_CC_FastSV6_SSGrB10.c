//------------------------------------------------------------------------------
// LG_CC_FastSV6_SSGrB10: connected components
//------------------------------------------------------------------------------
...
// University, and Gabriel Gomez, Texas A&M University.
...
// Modified by Gabriel Gomez, Texas A&M University: revised Reduce_assign to use
// GrB and GxB and LAGraph methods, with opaque GrB_Vector inputs.
...
(
    ...
    GrB_Vector ramp,        // ramp = [0:n] size n + 1
    ...
    GrB_Monoid min_m,       // GrB_MIN_MONOID_(integer type)
    char *msg
)
{
    bool done = false;
    while (true)
    {
        ...
        //----------------------------------------------------------------------
        // parent = min (parent, C*mngp) where C(i,j) is present if i=parent(j)
        //----------------------------------------------------------------------

        // This function computes the following, which is done explicitly in the 
        // Reduce_assign function in LG_CC_Boruvka:
        //
        //      for (j = 0 ; j < n ; j++)
        //      {
        //          uint64_t i = Px [j] ;
        ...
        LG_TRY (LAGraph_FastAssign(
            parent, NULL, min, parent, mngp, ramp, min_m, msg)) ;
...
)
{
    ...
    int64_t *range = NULL ;
    GrB_Index n, nvals, *ht_key = NULL, *Px = NULL,
...
// ]
    ...
    GRB_TRY (GrB_Vector_new (&ramp_v, GrB_INT64, n+1)) ;
    GRB_TRY (GrB_assign (ramp_v, NULL, NULL, 0, GrB_ALL, n+1, NULL)) ;
    GRB_TRY (GrB_apply (
...
// ] todo: the above will all be done as a single call to GxB_select.
        ...
        LG_TRY (fastsv (T, parent, mngp, &gp, &gp_new, t, ramp_v, eq, min,
            min_2nd, min_m, msg)) ;
        // update Px
...
// will become truly a read-only matrix.
    ...
    LG_TRY (fastsv (A, parent, mngp, &gp, &gp_new, t, ramp_v, eq, min, min_2nd,
        min_m, msg)) ;
