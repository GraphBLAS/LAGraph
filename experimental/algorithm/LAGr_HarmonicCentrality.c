...
#define HC_JIT_DEFINITIONS                \
"#define HLL_P 10                      \n"\
    "#define HLL_REGISTERS (1 << HLLP) \n"

#define HC_MAKE_STR(x) #x
#define HC_TO_STR(x) HC_MAKE_STR(x)
...
void fdb_hll_init(HLL *z, const uint64_t *x, GrB_Index i, GrB_Index j,
                  bool theta) {
    int64_t weight = *x;
...
JIT_STR(
void fdb_hll_merge(HLL *z, const HLL *x, const HLL *y) {
...
JIT_STR(
    void fdb_hll_delta(double *z, const HLL *x, const HLL *y) {
    *z = 0;
...
int64_t print_hll(char *string,       // value is printed to the string
...
int LAGr_HarmonicCentrality(
    ...
    GRB_TRY(GxB_Matrix_extract_Vector(_A, NULL, NULL, A, node_weights,
                                      node_weights, desc));
    ...
    GRB_TRY(GrB_Vector_assign_FP64(*scores, node_weights, NULL, 0.0, GrB_ALL, 0,
                                   GrB_DESC_S));
    ...
    GRB_TRY(
        GxB_Vector_extractTuples_Vector(NULL, flat_weight, node_weights, NULL));


    // init op: weight (INT64) at row index i → HLL seeded with 'weight' hashes
    GRB_TRY(GrB_IndexUnaryOp_new(&init_hlls,
                                 (GxB_index_unary_function)fdb_hll_init, hll_t,
                                 GrB_INT64, GrB_BOOL));

    // merge binary op (HLL, HLL) -> HLL  in-place: z == x required
    GRB_TRY(GxB_BinaryOp_new(&merge_hll_biop, (GxB_binary_function)fdb_hll_merge,
                             hll_t, hll_t, hll_t, "fdb_hll_merge",
                             FDB_HLL_MERGE_STR));

    // second op
    GRB_TRY(GxB_BinaryOp_new(&shallow_second, (GxB_binary_function)fdb_hll_second,
                             hll_t, GrB_BOOL, hll_t, "fdb_hll_second",
                             FDB_HLL_SECOND_STR));
    ...
    GRB_TRY(GxB_BinaryOp_new(&delta_hll, (GxB_binary_function)fdb_hll_delta,
                             GrB_FP64, hll_t, hll_t, "fdb_hll_delta",
                             FDB_HLL_DELTA_STR));
    ...
    GRB_TRY(
        GrB_Vector_assign_FP64(flat_scores, NULL, NULL, 0.0, GrB_ALL, 0, NULL));
    ...
    for (int d = 1; d <= CENTRALITY_MAX_ITER; d++) {
        ...
        GRB_TRY(GrB_mxv(new_sets, NULL, merge_hll_biop, merge_second, _A, old_sets,
                        NULL));

        GRB_TRY(GxB_unload_Vector_into_Container(old_sets, old_cont, NULL));
        // find the delta between last round and this one

        GRB_TRY(GrB_eWiseMult(delta_vec, NULL, NULL, delta_hll, new_sets,
                              old_cont->x, NULL));
        GRB_TRY(
            GrB_Vector_assign(old_cont->x, NULL, NULL, new_sets, GrB_ALL, 0, NULL));

        // old_set bitmap is the set of nodes with non-zero deltas
        GRB_TRY(
            GrB_apply(old_cont->b, NULL, NULL, GrB_IDENTITY_BOOL, delta_vec, NULL));
        GRB_TRY(GrB_Vector_reduce_INT64(&changes, NULL, GrB_PLUS_MONOID_INT64,
                                        old_cont->b, NULL));
        old_cont->nvals = changes;
        GRB_TRY(GxB_load_Vector_from_Container(old_sets, old_cont, NULL));

        // use the deltas to update the score
        GRB_TRY(GrB_apply(flat_scores, NULL, GrB_PLUS_FP64, GrB_DIV_FP64, delta_vec,
                          (double)d, NULL));
...
// TODO: add a descriptor? let nodes be values or indecies
...
int LAGr_HarmonicCentrality_exact(
    ...
    while (info != GxB_EXHAUSTED) {
        ...
        LG_TRY (LAGr_BreadthFirstSearch(&level, NULL, G_compact, nodeIdx, msg));

        // remove 0 to prevent div by 0
