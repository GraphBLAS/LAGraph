...
//****************************************************************************
void test_RegularReachabilityBasic (void)
{
    LAGraph_Init (msg) ;
    ...
    for (int k = 0 ; ; k++)
    {
        ...
        snprintf (testcase_name, LEN, "basic regular reachability %s", files[k].name) ;
        ...
        // Note the matrix rows/cols are enumerated from 0 to n meanwhile mtx
        // files are enumerated from 1 to n. Thus, when loading/comparing
        // results these values should be decremented/incremented.

        // Loat graph source nodes from the sources file
        GrB_Index s ;
        GrB_Index S[16] ;
        size_t ns = 0 ;
        ...
        OK (LAGraph_RegularReachability (&r, R, MAX_LABELS, QS, nqs,
                                         QF, nqf, G, S, ns, msg)) ;
        ...
        // Comare the results with expected values
...
TEST_LIST = {
    {"RegularReachabilityBasic", test_RegularReachabilityBasic},
    {NULL, NULL}
};
