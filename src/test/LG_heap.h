...
)
{
    ...
    for (int64_t name = 0 ; name < n ; name++)
    {
        ...
        {
            LG_ASSERT_MSG (p <= nheap,
                -2000, "position of this object is invalid") ; // RETVAL
            // object with this name is in the heap at position p
            LG_Element e = Heap [p] ;
            LG_ASSERT_MSG (e.name == name,
                -2000, "Heap is invalid") ;  // RETVAL
        }
    }
...
)
{
    ...
    for (int64_t p = 1 ; p <= nheap / 2 ; p++)
    {
        ...
        LG_ASSERT_MSG (! (pleft <= nheap && Heap [p].key > Heap [pleft].key),
            -2000, "the min-heap property is not satisfied") ; // RETVAL
